...
class HyperInverter(nn.Module):
    ...
    def forward(self, x, return_latents=False):
        ...
        # ======== Phase 1 ======== # "train an encoder E1 to encode the input image x to a content code w in W-space"

        # Obtain w code via W Encoder
        w_codes = self.w_encoder(x)  # bs x 1 x 512 # Encoder 1
        ...
        # ======== Phase 2 ======== # "further regress the residual weights to update the generator to faithfully reconstruct the input details"

        # Get w_bar code via W bar encoder
        w_bar_codes = self.w_bar_encoder(x) # Encoder 2 output, part 1

        # Get w image features
        w_images_resized = F.interpolate(w_images, size=(256, 256), mode="bilinear", align_corners=False)
        w_image_codes = self.w_bar_encoder(w_images_resized) # Encoder 2 output, part 2

        # Predict weights added to weights of StyleGAN2-Ada synthesis network
        predicted_weights = self.hypernet(w_image_codes, w_bar_codes)##### "Fusion + Feature Transform + Linear Mapping"
        ...
        for idx in range(bs):
            ...
            '''
            final_image = (
                self.decoder[idx].synthesis(w_code, noise_mode="const").squeeze(0)
            )
            '''
        ...
        #####return_data = [w_images, final_images]
        if return_latents:
